...
- 사용자가 **먹은 뒤 사진**을 함께 주면 두 사진을 비교해 먹은 비율을 추정한다.
- 사용자가 **"반 남김", "국물만 남김"** 처럼 말로 알려주면 그 값을 그대로 쓴다.
  사진 비교보다 못하지 않다 — 국물·밥은 남은 사진에서도 깊이가 안 보인다.
- **아무 말이 없으면 다 먹은 것으로 간주**하되, 답장에 "다 드신 걸로 기록했습니다"라고
  한 줄 남겨 사용자가 바로잡을 수 있게 한다. 남겼는지 매번 캐묻지는 말 것.

기록할 때 `kcal`·`c`·`p`·`f` 는 **실제 먹은 양**으로 넣고, 남긴 경우에만 `served`(차린 양 kcal)와
`ate`(먹은 비율)를 덧붙인다. 나중에 비율만 고치면 다시 계산할 수 있다.

```json
{"d":"2026-08-20","t":"점심","n":"돈까스","kcal":616,"c":67,"p":21,"f":28,"served":880,"ate":0.7}
```
